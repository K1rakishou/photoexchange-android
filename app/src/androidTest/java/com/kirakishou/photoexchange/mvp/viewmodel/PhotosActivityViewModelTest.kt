...
class PhotosActivityViewModelTest {
    ...
    fun test_load_page_of_uploaded_photos_should_not_delete_cached_uploaded_photos_and_concat_them_with_fresh_photos() {
        ...
        val values = viewModel.loadNextPageOfUploadedPhotos(lastId, photosCount)
            .test()
            .await()
            .values()
    ...
    fun test_load_page_of_received_photos_should_not_delete_cached_received_photos_and_concat_them_with_fresh_photos() {
        ...
        val values = viewModel.loadNextPageOfReceivedPhotos(lastId, photosCount)
            .test()
            .await()
            .values()
        ...
        //TODO: fix wrong ordering
        assertEquals(1L, photos[2].photoId)
        assertEquals("111", photos[2].uploadedPhotoName)
        assertEquals(2L, photos[1].photoId)
        assertEquals("222", photos[1].uploadedPhotoName)
        assertEquals(3L, photos[0].photoId)
        assertEquals("333", photos[0].uploadedPhotoName)
        assertEquals(4L, photos[3].photoId)
    ...
    fun test_load_page_of_gallery_photos_should_not_delete_cached_gallery_photos_and_concat_them_with_fresh_photos() {
        ...
        Mockito.`when`(apiClient.getGalleryPhotoInfo(userId, "3,2,1,4,5"))
        ...
        val values = viewModel.loadNextPageOfGalleryPhotos(lastId, photosCount)
            .test()
            .await()
            .values()
        ...
        //TODO: fix wrong ordering
        assertEquals(1L, photos[2].galleryPhotoId)
        assertEquals("111", photos[2].photoName)
        assertEquals(1, photos[2].galleryPhotoInfo!!.galleryPhotoId)
        ...
        assertEquals(3L, photos[0].galleryPhotoId)
        assertEquals("333", photos[0].photoName)
        assertEquals(3, photos[0].galleryPhotoInfo!!.galleryPhotoId)
