...
abstract class UploadedPhotoDao {
  ...
  @Query(
    "SELECT * FROM ${UploadedPhotoEntity.TABLE_NAME} " +
      "WHERE ${UploadedPhotoEntity.UPLOADED_ON_COLUMN} < :lastUploadedOn " +
      "ORDER BY ${UploadedPhotoEntity.UPLOADED_ON_COLUMN} DESC " +
      "LIMIT :count"
  )
  abstract fun getPage(lastUploadedOn: Long, count: Int): List<UploadedPhotoEntity>
  ...
  @Query(
    "UPDATE ${UploadedPhotoEntity.TABLE_NAME} " +
      "SET " +
  ...
  @Query(
    "DELETE FROM ${UploadedPhotoEntity.TABLE_NAME} " +
      "WHERE ${UploadedPhotoEntity.PHOTO_NAME_COLUMN} = :photoName"
  )
