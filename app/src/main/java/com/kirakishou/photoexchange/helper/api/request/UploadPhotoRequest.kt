...
class UploadPhotoRequest<T : StatusResponse>(
    ...
    override fun execute(): Deferred<T> {
        return async(coroutinePool.BG()) {
            val packet = SendPhotoPacket(location.lon, location.lat, userId)
            val photoFile = File(photoFilePath)

            if (!photoFile.isFile || !photoFile.exists()) {
                return@async UploadPhotoResponse.error(ServerErrorCode.NO_PHOTO_FILE_ON_DISK) as T
            }

            val body = getBody(photoId, photoFile, packet, callback)
            return@async extractResponse(apiService.uploadPhoto(body.part(0), body.part(1)).await() as Response<T>)
        }
    }
