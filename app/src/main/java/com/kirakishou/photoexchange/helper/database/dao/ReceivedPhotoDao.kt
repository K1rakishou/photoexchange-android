...
abstract class ReceivedPhotoDao {
  ...
  @Query(
    "SELECT * FROM ${ReceivedPhotoEntity.TABLE_NAME} " +
      "WHERE " +
      " ${ReceivedPhotoEntity.UPLOADED_ON_COLUMN} < :lastUploadedOn " +
      "AND " +
      " ${ReceivedPhotoEntity.INSERTED_ON_COLUMN} >= :deletionTime " +
      "ORDER BY ${ReceivedPhotoEntity.UPLOADED_ON_COLUMN} DESC " +
      "LIMIT :count"
  )
  abstract fun getPage(lastUploadedOn: Long, deletionTime: Long, count: Int): List<ReceivedPhotoEntity>
  ...
  abstract fun findOld(time: Long): List<ReceivedPhotoEntity>

  @Query("SELECT COUNT(*) FROM ${ReceivedPhotoEntity.TABLE_NAME}")
  abstract fun countAll(): Long

  @Query(
    "DELETE FROM ${ReceivedPhotoEntity.TABLE_NAME} " +
